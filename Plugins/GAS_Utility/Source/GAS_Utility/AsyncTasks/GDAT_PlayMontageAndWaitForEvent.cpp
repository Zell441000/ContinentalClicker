...
#include "AsyncTasks/GDAT_PlayMontageAndWaitForEvent.h"
...
UAbilitySystemComponentBase* UGDAT_PlayMontageAndWaitForEvent::GetTargetASC()
{
...
UGDAT_PlayMontageAndWaitForEvent* UGDAT_PlayMontageAndWaitForEvent::PlayMontageAndWaitForEvent(
...
{
	if (Ability == nullptr || !MontageToPlay)
	{
		EndTask();
		return;
	}
	...
	UAbilitySystemComponentBase* GDAbilitySystemComponent = GetTargetASC();
	...
	if (!Mesh && ActorInfo->SkeletalMeshComponent.IsValid())
		Mesh = GetMesh();

	if (!Mesh)
	{
		UE_LOG(LogTemp, Warning, TEXT("%s called on invalid Mesh"), *FString(__FUNCTION__));
		EndTask();
		return;
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("UGDAbilityTask_PlayMontageAndWaitForEvent::PlayMontageForMesh: Play montage %s"), *GetNameSafe(MontageToPlay));
	if (GDAbilitySystemComponent->PlayMontageForMesh(Ability, Mesh, SlotName, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection, bReplicateMontage, StartTimeSeconds) > 0.f)
	{
...
bool UGDAT_PlayMontageAndWaitForEvent::StopPlayingMontage(float OverrideBlendOutTime)
{
	if (GetMesh() == nullptr)
	{
		return false;
	}
	UAbilitySystemComponentBase* GSAbilitySystemComponent = GetTargetASC();
	...
	if (GSAbilitySystemComponent && Ability)
	{
		...
		//UE_LOG(LogTemp, Warning, TEXT("UGDAbilityTask_PlayMontageAndWaitForEvent::StopPlayingMontage: Stopping montage %s"), *GetNameSafe(MontageToPlay));

		GSAbilitySystemComponent->CurrentMontageStopForMesh(GetMesh(), SlotName, OverrideBlendOutTime);
		return true;
	}
...
USkeletalMeshComponent* UGDAT_PlayMontageAndWaitForEvent::GetMesh() const
{
	if (Mesh) return Mesh;
	if (Ability)
	{
