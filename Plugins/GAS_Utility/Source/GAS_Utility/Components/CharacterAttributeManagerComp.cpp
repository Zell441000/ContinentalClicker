...
void UCharacterAttributeManagerComp::BeginPlay()
{
	...
	if (APawn* PawnOwner = GetPawn<APawn>())
	{
		...
		{
			AbilitySystemComp = Cast<UAbilitySystemComponentBase>(
	GetOwner()->GetComponentByClass(UAbilitySystemComponentBase::StaticClass()));
		}
	}
...
void UCharacterAttributeManagerComp::SetupSlowGameplayChanger()
{
	if (SlowGameplayChanger == nullptr && AbilitySystemComp && CharacterOwner)
	{
		...
		SlowWalk_AttributeChanger->SetPriorityValue(100.0f);
		SlowRotation_AttributeChanger->SetPriorityValue(100.0f);
		SlowWalkAndRotation_AttributeChangerForWalk->SetPriorityValue(100.0f);
		SlowWalkAndRotation_AttributeChangerForRotation->SetPriorityValue(100.0f);
...
void UCharacterAttributeManagerComp::ApplySlowWalkSpeedFromDirection()
{
	if (CharacterOwner.IsValid() && AbilitySystemComp.IsValid())
	{
		const FVector InputVector = CharacterMovementComponent->GetCurrentAcceleration();
