...
public:
	...
	bool NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
	{
		...
		uint8 TypeInt = static_cast<uint8>(LastTypeSet);
		Ar << TypeInt;
		if (Ar.IsLoading())
		{
			LastTypeSet = static_cast<EGAS_Utility_VariableType>(TypeInt);
		}
		...
		case EGAS_Utility_VariableType::Variable_Object:
			for (auto& Obj : ObjectValues)
				Ar << Obj;
			break;
		case EGAS_Utility_VariableType::Variable_Tag:
			TagValue.NetSerialize(Ar, Map, bOutSuccess);
			TagValues.NetSerialize(Ar, Map, bOutSuccess);
			break;
		default:
			break;
		}
	...
	friend FArchive& operator<<(FArchive& Ar, FGAS_Utility_Variable& Variable)
	{
		uint8 TypeInt = static_cast<uint8>(Variable.LastTypeSet);
		Ar << TypeInt;
		if (Ar.IsLoading())
		{
			Variable.LastTypeSet = static_cast<EGAS_Utility_VariableType>(TypeInt);
		}

		bool bOutSuccess= true;
		switch (Variable.LastTypeSet)
		{
		case EGAS_Utility_VariableType::Variable_Bool:
			Ar << Variable.BoolValues;
			break;
		case EGAS_Utility_VariableType::Variable_Number:
			Ar << Variable.DoubleValues;
			break;
		case EGAS_Utility_VariableType::Variable_String:
			Ar << Variable.StringValues;
			break;
		case EGAS_Utility_VariableType::Variable_Vector:
			Ar << Variable.VectorValues;
			break;
		case EGAS_Utility_VariableType::Variable_Object:
			for (auto& Obj : Variable.ObjectValues)
				Ar << Obj;
			break;
		case EGAS_Utility_VariableType::Variable_Tag:
			
			Variable.TagValue.NetSerialize(Ar, nullptr, bOutSuccess);
			Variable.TagValues.NetSerialize(Ar, nullptr, bOutSuccess);
			break;
		default:
			break;
		}
...
template<>
struct TStructOpsTypeTraits<FGAS_Utility_Variable> : public TStructOpsTypeTraitsBase2<FGAS_Utility_Variable>
{
	enum
	{
...
public:
	...
	friend FArchive& operator<<(FArchive& Ar, FGAS_Utility_VariableContainerStruct& Variable)
	{
		Ar << Variable.VariablesNames;
		Ar << Variable.VariablesValues;
		return Ar;
	}
...
template<>
struct TStructOpsTypeTraits<FGAS_Utility_VariableContainerStruct> : public TStructOpsTypeTraitsBase2<FGAS_Utility_VariableContainerStruct>
{
	enum
	{
		WithNetSerializer = true,
