...
USTRUCT()
struct GAS_UTILITY_API FAbilityMeshMontage
{
	GENERATED_BODY()
...
public:
	...
	FAbilityMeshMontage() : Mesh(nullptr),SlotName(NAME_None), Montage(nullptr)
	{
	}

	FAbilityMeshMontage(class USkeletalMeshComponent* InMesh,FName InSlotName, class UAnimMontage* InMontage) 
		: Mesh(InMesh),SlotName(InSlotName), Montage(InMontage)
	{
	}
};
/**
...
public:
	...
	UAnimMontage* GetCurrentMontageForMesh(USkeletalMeshComponent* InMesh,FName SlotName);

	/** Call to set/get the current montage from a montage task. Set to allow hooking up montage events to ability events */
	virtual void SetCurrentMontageForMesh(USkeletalMeshComponent* InMesh,FName SlotName, class UAnimMontage* InCurrentMontage);
	...
	bool FindAbillityMeshMontage(USkeletalMeshComponent* InMesh, FName SlotName, FAbilityMeshMontage& InAbilityMontage);

	/** Immediately jumps the active montage to a section */
	UFUNCTION(BlueprintCallable, Category = "Ability|Animation")
	void MontageJumpToSectionForMesh(USkeletalMeshComponent* InMesh, FName SlotName, FName SectionName);

	/** Sets pending section on active montage */
	UFUNCTION(BlueprintCallable, Category = "Ability|Animation")
	void MontageSetNextSectionNameForMesh(USkeletalMeshComponent* InMesh, FName SlotName, FName FromSectionName, FName ToSectionName);
	...
	void MontageStopForMesh(USkeletalMeshComponent* InMesh, FName SlotName, float OverrideBlendOutTime = -1.0f);
