...
bool UGameplayAbilityBase::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	if (ActorInfo->AbilitySystemComponent.IsValid())
	{
		...
		if (GasComp)
		{
			if (GasComp->ForceExecuteAbilityBehaviourTaskTag.IsValid() && GasComp->ForceExecuteAbilityBehaviourTaskTag == GetAssetTags().First() && !IsActive())
				return true;
		}
...
UAnimMontage* UGameplayAbilityBase::GetCurrentMontageForMesh(USkeletalMeshComponent* InMesh, FName SlotName)
{
	FAbilityMeshMontage AbilityMeshMontage;
	if (FindAbillityMeshMontage(InMesh, SlotName, AbilityMeshMontage))
	{
...
void UGameplayAbilityBase::SetCurrentMontageForMesh(USkeletalMeshComponent* InMesh, FName SlotName, UAnimMontage* InCurrentMontage)
{
	...
	{
		CurrentAbilityMeshMontages.Add(FAbilityMeshMontage(InMesh, SlotName, InCurrentMontage));
	}
}

bool UGameplayAbilityBase::FindAbillityMeshMontage(USkeletalMeshComponent* InMesh, FName SlotName, FAbilityMeshMontage& InAbilityMeshMontage)
{
	for (FAbilityMeshMontage& MeshMontage : CurrentAbilityMeshMontages)
	{
		if (MeshMontage.Mesh == InMesh && MeshMontage.SlotName == SlotName)
		{
...
void UGameplayAbilityBase::MontageJumpToSectionForMesh(USkeletalMeshComponent* InMesh, FName SlotName, FName SectionName)
{
	check(CurrentActorInfo);

	UAbilitySystemComponentBase* const AbilitySystemComponent = Cast<UAbilitySystemComponentBase>(GetAbilitySystemComponentFromActorInfo_Ensured());
	if (AbilitySystemComponent->IsAnimatingAbilityForAnyMesh(this))
	{
		AbilitySystemComponent->CurrentMontageJumpToSectionForMesh(InMesh, SlotName, SectionName);
	}
}

void UGameplayAbilityBase::MontageSetNextSectionNameForMesh(USkeletalMeshComponent* InMesh, FName SlotName, FName FromSectionName, FName ToSectionName)
{
	check(CurrentActorInfo);
	...
	if (AbilitySystemComponent->IsAnimatingAbilityForAnyMesh(this))
	{
		AbilitySystemComponent->CurrentMontageSetNextSectionNameForMesh(InMesh, SlotName, FromSectionName, ToSectionName);
	}
}

void UGameplayAbilityBase::MontageStopForMesh(USkeletalMeshComponent* InMesh, FName SlotName, float OverrideBlendOutTime)
{
	check(CurrentActorInfo);
	...
	if (AbilitySystemComponent != nullptr)
	{
		...
		if (AbilitySystemComponent->IsAnimatingAbilityForAnyMesh(this))
		{
			AbilitySystemComponent->CurrentMontageStopForMesh(InMesh, SlotName, OverrideBlendOutTime);
		}
	}
}

void UGameplayAbilityBase::MontageStopForAllMeshes(float OverrideBlendOutTime)
{
	check(CurrentActorInfo);
