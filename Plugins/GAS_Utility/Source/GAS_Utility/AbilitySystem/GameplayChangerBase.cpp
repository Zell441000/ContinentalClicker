...
UAttributeChanger* UGameplayChangerBase::CreateAttributeChanger(FGameplayAttribute Attribute, TEnumAsByte<EAttributeChanger::Type> Operation, float Value)
{
	// Créer le nouveau AttributeChanger
	UAttributeChanger* NewAttributeChanger = NewObject<UAttributeChanger>();
	
	// Configurer l'AttributeChanger
	NewAttributeChanger->SetOperation(Operation);
	NewAttributeChanger->SetFloatValue(Value);
	NewAttributeChanger->GameplayAttribute = Attribute;
	NewAttributeChanger->GameplayChangerParent = this;

	// Vérifier si un array existe déjà pour cet attribut
	FAttributeChangerArray* ExistingArray = AttributeChangers.Find(Attribute);
	if (ExistingArray)
	{
		// Ajouter à l'array existant
		ExistingArray->ArrayList.Add(NewAttributeChanger);
	}
	else
	{
		// Créer un nouveau array
		FAttributeChangerArray NewArray;
		NewArray.ArrayList.Add(NewAttributeChanger);
		AttributeChangers.Add(Attribute, NewArray);
	}

	// Déclencher le refresh de l'attribut
	TriggerRefreshAttribute(Attribute);

	return NewAttributeChanger;
}
